...
public class courseOptions 
{
	...
	public static void  searchOption() throws Exception {
		...
		List<Course> courses = courseData.search("CS-103");
		for (int i=0; i<courses.size(); i++) {
			System.out.println(courses.get(i).toString());
		}
		try {
	...
	public static void  saveOption() throws Exception {
		...
		course.setCOURSE_CODE("CS-103");
		course.setCOURSE_TITLE("Java");
		course.setCOURSE_DESC("course covers core java");
		course.setCOURSE_CREDITHOUR(4);
		System.out.println("Enter Course PREREQUISIT ID: ");
		int preID = scan.nextInt();
		Course preREQ = courseData.findOne(preID);
		course.setCOURSEPRERQUIST_ID(preREQ);
